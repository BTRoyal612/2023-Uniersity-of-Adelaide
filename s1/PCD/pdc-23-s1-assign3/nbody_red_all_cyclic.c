 ...
 *
 * Compile:  gcc -g -Wall -fopenmp -o nbody_red_all_cyclic 
 *              nbody_red_all_cyclic.c -lm
 *           To turn off output (e.g., when timing), define NO_OUTPUT
 *           To get verbose output, define DEBUG
 *
 * Run:      ./nbody_red_all_cyclic 
...
/* In this version of the program (nbody_red_all_cyclic.c), all inner
 * loops are scheduled using a cyclic distribution. This is achieved 
 * using the 'omp for schedule(static,1)' directive for each loop, which 
 * distributes iterations to threads in a cyclic partition. This 
 * differs from the default version where the OpenMP's default scheduling
 * policy is used. 
 */
...
#  pragma omp parallel num_threads(thread_count) default(none) \
      ...
      private(step, part, t)
   {
      ...
      /* All inner loops in this program use cyclic scheduling. 
         To achieve this, 'schedule(static, 1)' is added to each 
         'omp for' directive. This ensures that iterations are 
         distributed to threads in a cyclic manner.
       */

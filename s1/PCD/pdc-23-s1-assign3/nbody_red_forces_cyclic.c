/* File:     nbody_red_forces_cyclic.c
 *
 ...
 *
 * Compile:  gcc -g -Wall -fopenmp -o nbody_red_forces_cyclic 
 *              nbody_red_forces_cyclic.c -lm
 *           To turn off output (e.g., when timing), define NO_OUTPUT
 *           To get verbose output, define DEBUG
 *
 * Run:      ./nbody_red_forces_cyclic 
...
/* In this version of the program (nbody_red_forces_cyclic.c), the first
 * phase of the forces computation loop is scheduled using a cyclic 
 * distribution via the 'omp for schedule(static,1)' directive. This ensures 
 * that iterations are distributed to threads in a cyclic partition. The 
 * other inner loops use OpenMP's default scheduling policy. 
 */
...
#  pragma omp parallel num_threads(thread_count) default(none) \
      ...
      private(step, part, t)
   {
      ...
      /* Only the inner loop that computes forces in this program 
         uses cyclic scheduling. To implement this, 'schedule(static, 1)' 
         is added to the 'omp for' directive in the force computation loop. 
         This ensures that iterations are distributed among threads in a 
         cyclic manner.
      */
      for (step = 1; step <= n_steps; step++) {
         t = step*delta_t;
//       memset(loc_forces + my_rank*n, 0, n*sizeof(vect_t));
         /* 'omp for' directive for default scheduling */
#        pragma omp for
...
#        endif
          ...
          * Compute_force(n-2, . . .) */
         /* 'omp for schedule(static,1)' directive for cyclic scheduling */

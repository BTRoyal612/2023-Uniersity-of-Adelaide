 ...
 *
 * Compile:  gcc -g -Wall -fopenmp -o nbody_red_default 
 *              nbody_red_default.c -lm
 *           To turn off output (e.g., when timing), define NO_OUTPUT
 *           To get verbose output, define DEBUG
 *
 * Run:      ./nbody_red_default 
...
/* In this version of the program (nbody_red_default.c), all inner 
 * loops are scheduled using OpenMP's default scheduling policy. 
 * As such, we simply use the 'omp for' directive to parallelize these 
 * loops. This directive allows OpenMP to automatically manage the 
...
#  pragma omp parallel num_threads(thread_count) default(none) \
      ...
      private(step, part, t)
   {
      ...
      /* All the inner loops in this program use default scheduling. 
         To implement this, we use the 'omp for' directive for all 
         inner loops. This ensures hat iterations are distributed 
         among threads in a block manner.
      */
